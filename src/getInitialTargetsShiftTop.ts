...
export const getInitialTargetsShiftTop = (source: TreeNode, targets: TreeNode[], settings: Settings, map: TreeMap) => {
  return (
    targets.reduce((totalHeight, target, index) => {
      ...
      totalHeight += target.height + (index === targets.length - 1 ? 0 : target.marginBottom);
