...
export const getInitialTargetsShiftLeft = (source: TreeNode, targets: TreeNode[], settings: Settings, map: TreeMap) => {
  return (
    targets.reduce((totalWidth, target, index) => {
      ...
      totalWidth += target.width + (index === targets.length - 1 ? 0 : target.marginRight);
