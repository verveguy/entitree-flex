    ...
    getFromMap(root[settings.nextBeforeAccessor], map)
      ...
      .forEach((currentNode, nextBeforeIndex, nextBefores) => {
        ...
        if (settings.orientation === 'vertical') {
          currentNode.x = previousNode.x - currentNode.width - currentNode.marginRight;
          //align vertically
          currentNode.y = root.y + root.height / 2 - currentNode.height / 2;

          const currentBottomY = currentNode.y + currentNode.height + currentNode.marginBottom;
          if (currentNode.y <= (root.groupTopY ?? currentNode.y)) root.groupTopY = currentNode.y;
          if (currentBottomY >= (root.groupBottomY ?? currentBottomY)) root.groupBottomY = currentBottomY;
        } else {
          currentNode.y = previousNode.y - currentNode.height - currentNode.marginBottom;
          //align horizontally
          ...
          if (currentRightEdgeX > root.groupRightX) root.groupRightX = currentRightEdgeX;
        }

        checkContourOverlap(contour, currentNode, settings);
      });
};
