...
export const addRootSpousesPositions = <T>(
  ...
  if (root[settings.nextAfterAccessor])
    getFromMap(root[settings.nextAfterAccessor], map).forEach((currentNode, nextAfterIndex, nextAfters) => {
      ...
      if (settings.orientation === 'vertical') {
        ...
        const bottomEdgeY = currentNode.y + currentNode.height + currentNode.marginBottom;
        ...
        const currentRightEdgeX = currentNode.x + currentNode.width + currentNode.marginRight;
